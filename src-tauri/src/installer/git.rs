...
pub fn command_spec(config: &AppConfig) -> InstallCommandSpec {
    InstallCommandSpec {
        ...
        success_suggestion: Some("Git for Windows 安装完成，正在重新检测 Git / Git Bash。".to_string()),
        failure_suggestion: Some(
            "如果日志提示权限不足或 UAC 被拒绝，请以管理员身份重启后重试。".to_string(),
        ),
        detect_after: vec!["git".to_string()],
    }
}
...
mod tests {
    ...
    #[test]
    fn adds_manual_proxy_warning_for_git() {
